class_name TextureWizard
extends RefCounted # texture_wizard.gd
...
static func get_png_rect_from_texture(texture: Texture2D) -> Rect2i:
	...
	assert(image is Image, "TextureWizard->get_png_rect_from_texture: The image from the texture is null, the texture it's invalid")
