...
extends RefCounted # node_remover.gd
## Easy to use functions for Node removal that can be called from anywhere in the project.
...
static func remove_and_queue_free_children(node:Node, except:Array = [])->void:
	if node.get_child_count() == 0:
		return
	var childrens = node.get_children().filter(func(child): \
												return not child.get_class() in except)
	childrens.reverse()
