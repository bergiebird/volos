...
extends Line2D # wiggly_appendage_2d.gd

# DOCS: https://github.com/Tameno-01/GodotWigglyAppendage2D
...
func _physics_process(delta):
	if only_process_when_visible and not is_visible_in_tree(): return

	for i in range(physics_points.size()):
		if i == 0: _process_root_point(physics_points[i], delta)
		else: _process_point(physics_points[i], delta, i)
	_update_line()
...
func reset(point_count: int = segment_count + 1) -> void:
	physics_points = []
	var starting_pos: Vector2 = get_global_position()
