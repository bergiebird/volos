...
var current_lvl_id :int:
	set = set_current_lvl_id

func set_current_lvl_id(value :int)->void:
...
func _advance_level()->bool:
	...
	current_lvl_id += 1
	GameState.set_current_level_node(current_lvl_id)
	return true
...
func get_current_lvl_id()->int:
	return current_lvl_id if force_level == -1\
			 else force_level

func load_current_level_node()->void:
	level_list_loader.load_level(get_current_lvl_id())

func _advance_and_reload()->void:
	var _prior_level_id:int = get_current_lvl_id()
	_advance_level()
...
func _load_level_complete_screen_or_next_level()->void:
	if level_won_scene:
		var instance :Node = level_won_scene.instantiate()
		get_tree().current_scene.add_child(instance)
...
func is_on_last_level()->int:
	return get_current_lvl_id() + 1 >= level_list_loader.files.size()
