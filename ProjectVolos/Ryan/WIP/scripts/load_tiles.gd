...
var string_array: Array[String] = ["n","ne","se","s","sw","w","nw","dark"]
	...
	for tileset in tile_sets:
		...
		while old_count > 0:
			...
			while atlas_count > 0:
				...
				instance.tap_content_index = total_count - old_count
				instance.coords = coord
				instance.tab = " ".join(name_of_file.split("-"))
				self.get_child(total_count - old_count).get_child(0).add_child(instance)
